...
class TCLFRLoadCommonPar(object):
    """docstring for TCLFRLoadCommonPar"""

    def __init__(self, sequence_cnt, source_id):
        super(TCLFRLoadCommonPar, self).__init__()
    ...
    def buildTCParameters(self):
        ...
        val = 0
        # BW
        val = val + ((self.sy_lfr_bw & 0x1) << 5)
        # SP0
        val = val + ((self.sy_lfr_sp0 & 0x1) << 4)
        # SP1
        val = val + ((self.sy_lfr_sp1 & 0x1) << 3)
        # R0
        val = val + ((self.sy_lfr_r0 & 0x1) << 2)
        # R1
        val = val + ((self.sy_lfr_r1 & 0x1) << 1)
        # R2
        val = val + (self.sy_lfr_r2 & 0x1)
    ...
    def setSY_LFR_R2(self, sy_lfr_r2):
        self.sy_lfr_r2 = sy_lfr_r2
        self.ccsdsPacket = self.buildCCSDSPacket()
